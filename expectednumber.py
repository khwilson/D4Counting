...
def do_unram_fields():
  ...
  Returns:
    list[(int, int, int)]: Every entry in the list represents some number of
...
def compute():
  ...
  Returns:
    dict[(int, int), int]: A dictionary whose keys are the (d, q) invariants of the
...
if __name__ == '__main__':
  ...
  print("Total weight:", sum(val / 2**(d+q+3) for (d, q), val in outcomes.items()))
  print()
  print("Latex table:")
